    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for it, (input_data, labels) in enumerate(self.train_loader):
                ...
                time_local_reconstruction_loss = 0.0
                time_global_reconstruction_loss = 0.0
                space_local_reconstruction_loss = 0.0
                space_global_reconstruction_loss = 0.0
                time_contrastive_loss = 0.0
                space_contrastive_loss = 0.0
                series, prior, series_1, prior_1= self.model(x,x_local_size, x_global_num,op,it,x_in)

                for u in range(len(prior)):
                    time_local_reconstruction_loss +=self.criterion(series[u],x)
                    time_global_reconstruction_loss += self.criterion(prior[u],x)
                    space_local_reconstruction_loss += self.criterion(series_1[u],x)
                    space_global_reconstruction_loss += self.criterion(prior_1[u],torch.mean(x_in[u],dim=-1))
                    time_contrastive_loss += self.criterion(series[u],prior[u])
                    space_contrastive_loss += self.criterion(series_1[u],prior_1[u])
                time_local_reconstruction_loss = time_local_reconstruction_loss / len(prior)
                time_global_reconstruction_loss =  time_global_reconstruction_loss / len(prior)
                space_local_reconstruction_loss = space_local_reconstruction_loss / len(prior)
                space_global_reconstruction_loss =  space_global_reconstruction_loss / len(prior)
                time_contrastive_loss = time_contrastive_loss / len(prior)
                space_contrastive_loss =space_contrastive_loss/ len(prior)
                loss = (time_local_reconstruction_loss + time_global_reconstruction_loss + time_contrastive_loss)*self.r + (space_contrastive_loss + space_local_reconstruction_loss +space_global_reconstruction_loss)*(1-self.r)
                loss.backward()
    ...
    def test(self):
        ...
        for it, (input_data, labels) in enumerate(self.train_loader):
            ...
            for index, localsize in enumerate(self.local_size):
                ...
                front1 = self.mul_num // 2
                back1 = self.mul_num - front1
                in_x = torch.cat((in_size[:, :, :, site - front:site], in_size[:, :, :, site + 1:site + back]),
                                 dim=-1)  # 去除自身的局部
            ...
            for u in range(len(prior)):
                ...
                # time_local_reconstruction_loss += torch.mean(self.criterion_keep(series[u], x),dim=-1)
                # time_global_reconstruction_loss += torch.mean(self.criterion_keep(prior[u], x),dim=-1)
                # space_local_reconstruction_loss += torch.mean(self.criterion_keep(series_1[u], x),dim=-1)
                # space_global_reconstruction_loss += torch.mean(self.criterion_keep(prior_1[u], torch.mean(x_in[u], dim=-1)),dim=-1)
                time_contrastive_loss += torch.mean(self.criterion_keep(series[u],prior[u]),dim=-1)
                space_contrastive_loss += torch.mean(self.criterion_keep(series_1[u],prior_1[u]),dim=-1)

            metric = minmax_norm(time_contrastive_loss)*self.r + minmax_norm(space_contrastive_loss)*(1-self.r)
            # metric = minmax_norm(time_local_reconstruction_loss+time_global_reconstruction_loss) * self.r + minmax_norm(space_local_reconstruction_loss+space_global_reconstruction_loss) * (1 - self.r)
        ...
        for it, (input_data, labels) in enumerate(self.thre_loader):
            ...
            for u in range(len(prior)):
                ...
                # time_global_reconstruction_loss += torch.mean(self.criterion_keep(prior[u], x), dim=-1)
                # space_local_reconstruction_loss += torch.mean(self.criterion_keep(series_1[u], x), dim=-1)
                # space_global_reconstruction_loss += torch.mean(
                #     self.criterion_keep(prior_1[u], torch.mean(x_in[u], dim=-1)), dim=-1)
                time_contrastive_loss += torch.mean(self.criterion_keep(series[u],prior[u]),dim=-1)
                space_contrastive_loss += torch.mean(self.criterion_keep(series_1[u],prior_1[u]),dim=-1)

            metric = minmax_norm(time_contrastive_loss)*self.r + minmax_norm(space_contrastive_loss)*(1-self.r)
            # metric = minmax_norm(
            #     time_local_reconstruction_loss + time_global_reconstruction_loss) * self.r + minmax_norm(
            #     space_local_reconstruction_loss + space_global_reconstruction_loss) * (1 - self.r)
        ...
        #add noise
        rate = 0
        print('Noiserate',rate)
        num_noisy_elements = int(rate * len(attens_energy))
        noisy_indices = np.random.choice(len(attens_energy), num_noisy_elements, replace=False)
        noise_level = 1 * attens_energy.std()
        noise = np.random.normal(0, noise_level, num_noisy_elements)
        attens_energy.flat[noisy_indices] += noise
